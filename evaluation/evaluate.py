"""
OpenCustom Benchmark Evaluation Script

Evaluates personalized text-to-video generation across multiple metrics:
  - CLIP image/text similarity (global and regional)
  - DINO image similarity (global and regional)
  - Motion smoothness
  - Dynamic degree
...
Usage:
    torchrun --nproc_per_node=4 --standalone evaluate.py \
        ...
        --image_dir <path_to_reference_images> \
        --json_path benchmark/video_mapping_coco.json \
        --prompt_json benchmark/caption_mapping_coco.json \
        --mode default --n_frames 16 \
...
def parse_args():
    parser = argparse.ArgumentParser(description='OpenCustom Evaluation', formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument(
        "--output_path",
        type=str,
